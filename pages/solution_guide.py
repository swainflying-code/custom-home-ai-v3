...
━ Prompt 1：主空间全景 ━
用途：方案主效果图，展示整体空间氛围与设计语言
必须涵盖以下全部维度（缺一不可）：

[EN] Positive:
{{严格按顺序填写，用逗号分隔：
  [空间] {{户型形状，如 L-shaped kitchen / galley kitchen}} + {{面积感知，如 compact 6sqm / spacious 12sqm}} + {{层高，如 2.7m ceiling}}
  [视角] wide-angle interior photography, eye-level view, 16:9 composition
  [风格] {{来自①的一句话定调对应英文风格词，如 wabi-sabi minimalism / japandi / contemporary Italian luxury}}
  [配色] {{来自①的三色方案英文描述，如 warm ivory + brushed titanium + matte black accents}}
  [材质] {{台面材质英文名}} countertop, {{柜体材质英文名}} cabinet, {{五金英文名}} hardware
  [参考图元素] {{从参考图中提取≥2个核心视觉特征，如 open shelving with vertical ribbed panel / curved handleless door / recessed LED strip under cabinet}}
  [采光] {{来自C的采光信息，如 south-facing window, abundant natural light / limited natural light, warm recessed lighting}}
  [品质参数] photorealistic, 8K, ultra-detailed, professional interior design render, --ar 16:9
}}

[EN] Negative:
{{必须排除：cartoon, illustration, 3D wireframe, overexposed, watermark, cluttered, {{与风格相矛盾的元素，如选了极简则加 ornate decoration, baroque, heavy molding}}, {{与配色相矛盾的颜色}}}}

[CN 可选]:
{{中文版，含空间、风格、配色、材质、参考图来源元素的对应描述}}


━ Prompt 2：材质细节特写 ━
用途：展示核心材质碰撞点、工艺细节与触感质感
必须涵盖以下全部维度（缺一不可）：

[EN] Positive:
{{严格按顺序填写：
  [焦点] close-up shot of {{最核心材质碰撞面，如 stainless steel countertop meets matte wood cabinet edge}}
  [工艺细节] {{工艺名称，如 hairline finish / mirror polished / PVD coating / embossed texture}}
  [参考图元素] {{从参考图中提取≥1个细节特征，如 visible weld seam / integrated sink curve / shadow gap detail}}
  [光线] {{硬光/软光描述，如 directional studio light highlighting material texture / soft diffused light}}
  [品质参数] macro photography, f/2.8, 4K, ultra-sharp, --ar 4:3
}}

[EN] Negative:
{{cartoon, blur, overexposed, busy background, people, {{与材质相矛盾的描述}}}}

[CN 可选]:
{{中文版，点明材质碰撞关系和参考图来源细节}}


━ Prompt 3：宠物场景生活感（仅有宠物时生成） ━
用途：展示养宠家庭真实使用场景，体现设计的生活感

[EN] Positive:
{{严格按顺序填写：
  [宠物] {{精确品种+数量，如 one golden retriever / two British shorthair cats}}（必须从客户信息精确提取，禁止用 "a pet"）
  [场景] {{宠物使用场景，如 cat exploring lower cabinet shelf / dog resting near kitchen island}}
  [空间] {{与Prompt1一致的空间背景}} kitchen, {{关键设计特征}}
  [材质] {{宠物友好设计点，如 scratch-resistant stainless steel door panel / easy-clean matte surface}}
  [生活感] lifestyle photography, natural candid moment, warm ambient light
  [品质参数] photorealistic, 8K, --ar 3:2
}}

[EN] Negative:
{{cartoon, posed awkwardly, low quality, wrong breed, dark, cluttered}}

[CN 可选]:
{{中文版，点明宠物品种、空间场景和设计友好点}}
...
4. 【参考图解读】如有参考图，必须：①在板块①风格方向中引用参考图的具体视觉特征（门型/材质肌理/色彩组合/构成语言/工艺细节，至少2点）；②在板块⑦所有生图提示词的 [EN] Positive 中，用英文术语显式写出来自参考图的元素（不得使用"reference image style"或"similar to reference"等模糊写法）。无参考图时跳过，不生成、不标注。
...
Step 4：注入参考图元素（有参考图时强制执行，无参考图时跳过）
  → 逐图分析，提取≥2个可直接用于提示词的具体视觉特征
  → 可提取的特征维度示例：
     · 门型/把手：handleless / recessed grip / arch door / fluted vertical panel
     · 材质/肌理：ribbed oak veneer / honed marble slab / brushed brass inlay / concrete-look laminate
     · 色彩组合：warm ivory + deep navy island / all-white with blackened steel accents
     · 构成语言：floating cabinet / open shelving above counter / full-height pantry / integrated column
     · 光线/阴影：under-cabinet LED strip / skylight wash / pendant cluster over island
     · 工艺细节：shadow gap joint / mitred corner / integrated handle channel / visible weld seam
  → 禁止用模糊描述（如"modern style from reference" / "similar to reference image"）
  → 提取结果必须显式写入 Prompt 1 的 [参考图元素] 和 Prompt 2 的 [参考图元素] 字段
...
<button id="copy-btn" onclick="
  navigator.clipboard.writeText(`{_escaped}`)
    .then(() => {{
      this.textContent = '✅ 已复制';
      this.classList.add('done');
      setTimeout(() => {{ this.textContent = '📋 一键复制全部分析结果'; this.classList.remove('done'); }}, 2500);
    }})
    .catch(() => {{
      this.textContent = '⚠️ 复制失败，请手动选取';
    }});
">📋 一键复制全部分析结果</button>
""", height=52)
